...
namespace Nop.Plugin.Api.Common.DTOs.Product.Validator
{
    ...
    {
        public ProductSpecificationAttributeDtoValidator(string httpMethod,
            Dictionary<string, object> passedPropertyValuePaires)
        {
            ...
            else if (httpMethod.Equals("put", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                if (passedPropertyValuePaires.ContainsKey("product_id")) ApplyProductIdRule();

                if (passedPropertyValuePaires.ContainsKey("attribute_type_id")) ApplyAttributeTypeIdRule();
        ...
        {
            var specificationAttributeTypes =
                (SpecificationAttributeType[])Enum.GetValues(typeof(SpecificationAttributeType));
            RuleFor(x => x.AttributeTypeId)
                .InclusiveBetween((int)specificationAttributeTypes.First(), (int)specificationAttributeTypes.Last())
        ...
        {
            RuleFor(x => x.SpecificationAttributeOptionId).GreaterThan(0)
                .WithMessage("invalid specification attribute option id");
        }
    }
}