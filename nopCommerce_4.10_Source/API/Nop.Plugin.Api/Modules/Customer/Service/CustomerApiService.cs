    ...
    public class CustomerApiService : ICustomerApiService
    {
        ...
        public CustomerDto GetCustomerById(int id, bool showDeleted = false)
        {
            ...
            if (customerAttributeMappings.Count > 0)
            {
                ...
                int defaultStoreLanguageId = GetDefaultStoreLangaugeId();
                ...
                foreach (CustomerAttributeMappingDto mapping in customerAttributeMappings)
                {
        ...
        private int GetDefaultStoreLangaugeId()
        {
            ...
            int defaultLanguageId = _storeContext.CurrentStore.DefaultLanguageId;
            ...
            int page = Configurations.DefaultPageValue, int limit = Configurations.DefaultLimit, string order = Configurations.DefaultOrder)
        {
            ...
            // Get the default language id for the current store.
            int defaultLanguageId = GetDefaultStoreLangaugeId();

            foreach (IGrouping<int, CustomerAttributeMappingDto> group in customerAttributeGroupsList)
            {
            ...
            int limit = Configurations.DefaultLimit, int page = Configurations.DefaultPageValue, string order = Configurations.DefaultOrder)
        {
            ...
            {
                if (searchParams.ContainsKey(FirstName)) allRecordsGroupedByCustomerId = GetCustomerAttributesMappingsByKey(allRecordsGroupedByCustomerId, FirstName, searchParams[FirstName]);

                if (searchParams.ContainsKey(LastName)) allRecordsGroupedByCustomerId = GetCustomerAttributesMappingsByKey(allRecordsGroupedByCustomerId, LastName, searchParams[LastName]);

                if (searchParams.ContainsKey(LanguageId)) allRecordsGroupedByCustomerId = GetCustomerAttributesMappingsByKey(allRecordsGroupedByCustomerId, LanguageId, searchParams[LanguageId]);

                if (searchParams.ContainsKey(DateOfBirth)) allRecordsGroupedByCustomerId = GetCustomerAttributesMappingsByKey(allRecordsGroupedByCustomerId, DateOfBirth, searchParams[DateOfBirth]);

                if (searchParams.ContainsKey(Gender)) allRecordsGroupedByCustomerId = GetCustomerAttributesMappingsByKey(allRecordsGroupedByCustomerId, Gender, searchParams[Gender]);
            }
        ...
        private static CustomerDto Merge(IList<CustomerAttributeMappingDto> mappingsForMerge, int defaultLanguageId)
        {
            ...
            CustomerDto customerDto = mappingsForMerge.First().Customer.ToDto();
        ...
        private Dictionary<string, string> ParseSearchQuery(string query)
        {
            ...
            List<string> fieldValueList = Regex.Split(query, splitPattern).Where(s => s != string.Empty).ToList();

            if (fieldValueList.Count < 2) return parsedQuery;

            for (var i = 0; i < fieldValueList.Count; i += 2)
            {
                string field = fieldValueList[i];
                string value = fieldValueList[i + 1];
        ...
        private void SetNewsletterSubscribtionStatus(IList<CustomerDto> customerDtos)
        {
            ...
            IEnumerable<string> allNewsletterCustomerEmail = GetAllNewsletterCustomersEmails();

            foreach (CustomerDto customerDto in customerDtos) SetNewsletterSubscribtionStatus(customerDto, allNewsletterCustomerEmail);
        }
        ...
        private void SetNewsletterSubscribtionStatus(BaseCustomerDto customerDto, IEnumerable<string> allNewsletterCustomerEmail = null)
        {
            ...
            if (allNewsletterCustomerEmail != null && allNewsletterCustomerEmail.Contains(customerDto.Email.ToLowerInvariant())) customerDto.SubscribedToNewsletter = true;
        }
    }
}